export class App {
    constructor() {
        ...
        this.$btnDeleteNote = document.querySelector('.toolbar-delete');
    ...
    handleClickTooltip({ target }) {
        ...
        this.noteEditColor(color);
    }

    handleClickDeleteNote({ target }) {
        if (target.classList.contains('toolbar-delete')) {
            this.notes = this.notes.filter(({ id }) => id !== Number(this.id));
            this.render();
        };
    }
    ...
    noteEditColor(color) {
        this.notes = this.notes.map((note) =>
            note.id === Number(this.id) ? { ...note, color } :
            note
        );
        this.render();
    ...
    openToltip({ target }) {
        ...
        this.id = target.parentElement.parentElement.dataset.id;
    ...
    openModal({ target }) {
        const isNoteClicked = target.closest('.note') && !target.classList.contains('toolbar-delete');
