...
function checkCashRegister(price, cash, cid) {
  ...
  filterCid.forEach(elem => {
    ...
    if (amount !== 0) {change.push([curr], amount / 100);}

  });

  // if ((cidSum / 100) - (changeSumCheck / 100) === 0) {
  //   return status = 'CLOSED';
  //
  // } else if ((cidSum / 100) - (changeSumCheck / 100) > 0) {
  //   return status = 'OPEN';
  // }
  //
  // else if ((changeSumCheck / 100)-(cidSum / 100)  < 0) {
  //   return status = 'INSUFFICIENT_FUNDS';
  // }

 return ((changeSumCheck / 100)-(cidSum / 100)> (cidSum / 100));
}
