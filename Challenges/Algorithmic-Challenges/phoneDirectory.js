function phone(strng, num) {
  const formated = strng.split("\n").filter((data) => data.includes(num));

  if (formated.length >= 2) {
    return `Error => Too many people: ${num}`;
  }
  if (!Boolean(formated.length)) {
  ...
  const result = formated
    ...
    .reduce((acc, val) => {
      if (val.match(/(?<=<).*(?=>)|(.*?)>|<(.*?)/gim)) {
        val = val.replace(/<|>/gim, "");
        acc["name"] = acc["name"] ? acc["name"] + ` ${val}` : val;
      } else {
        val = val.replace(/[^\w.-]+/gim, " ").replace("_", " ");
        acc["add"] = acc["add"] ? acc["add"] + ` ${val}` : val;
      }
      return acc;
    }, {});
  return `Phone => ${num}, Name => ${result.name}, Address => ${result.add}`.replace(
    /[ ]{2,}/gim,
    " "
  );
}
