function travel(r, zipcode) {
  ...
  const addr = list.reduce((acc, direc) => {
    let zip = direc.slice(-8);

    const street = direc.split(" ").slice(1, -2).join(" ");

    if (acc[zip]) {
      acc[zip].street.push(street);
      acc[zip].houseNumber.push(direc.split(" ")[0]);
      return acc;
    }
    acc[zip] = { street: [street], houseNumber: [direc.split(" ")[0]] };

    return acc;
  }, {});

  if (!addr[zipcode]) return `${zipcode}:/`;

  return `${zipcode}:${addr[zipcode].street.join(",")}/${addr[
    zipcode
  ].houseNumber.join(",")}`;
}
